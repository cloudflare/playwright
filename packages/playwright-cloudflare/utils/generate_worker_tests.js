...
// generate workerTests/snapshots.ts file
const imgs = ['page', 'library'].flatMap(dir => listFiles(path.join(sourceTestsDir, dir), { recursive: true }))
  .filter(file => file.endsWith('-chromium.png'))
  .map(file => [path.relative(sourceTestsDir, file).replace(/\\/g, '/'), fs.readFileSync(file, 'base64')]);

writeFile(path.join(workerTestsDir, 'snapshots.ts'), `import path from 'path';
import fs from '@cloudflare/playwright/fs';

function load(file: string, base64: string) {
  const fullPath = '/' + file;
  fs.mkdirSync(path.dirname(fullPath), { recursive: true });
  fs.writeFileSync(fullPath, base64, { encoding: 'base64' });
}

${imgs.map(([imgPath, base64]) => `load(${JSON.stringify(imgPath)}, ${JSON.stringify(base64)});`).join('\n')}
`);
...
writeFile(path.join(workerTestsDir, 'index.ts'), `import "./snapshots";
